    ...
    public static class ExpRate extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length > 1) {
                ...
                c.getPlayer().dropMessage(6, "经验率修改为: " + rate + "倍");
            } else {
                c.getPlayer().dropMessage(6, "使用方法: !exprate 倍率 [all]");
            }
            return 1;
        }
    }
    ...
    public static class MesoRate extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length > 1) {
                ...
                c.getPlayer().dropMessage(6, "冒险币爆率修改为 " + rate + "倍");
            } else {
                c.getPlayer().dropMessage(6, "使用方法: !mesorate 倍率 [all]");
            }
            return 1;
        }
    }
...
//关闭服务器
    public static class Shutdown extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage(6, "服务器正在关闭...");
            if (t == null || !t.isAlive()) {
                t = new Thread(ShutdownServer.getInstance());
				ShutdownServer.getInstance().shutdown();
                t.start();
            } else {
                c.getPlayer().dropMessage(6, "关闭操作仍未完成,请稍等.");
            }
            return 1;
        }
    }
//在一定时间关闭服务器
    public static class ShutdownTime extends Shutdown {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "服务器将在 " + minutesLeft + " 分内关闭");
            if (ts == null && (t == null || !t.isAlive())) {
                ...
                ts = EventTimer.getInstance().register(new Runnable() {
                    ...
                    public void run() {
                        ...
                        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "服务器将在 " + minutesLeft + " 分内关闭. 请安全下线."));
                        minutesLeft--;
                    }
                }, 60000);
            } else {
