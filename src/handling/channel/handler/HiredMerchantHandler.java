               ...
               c.getSession().write(PlayerShopPacket.merchItemStore_ItemData(pack));
                ...
                for (final Item item : pack.getItems()) {
                    if(c.getPlayer().getInventory(GameConstants.getInventoryType(item.getItemId())).isFull()){
                        c.removeClickedNPC();
                        c.getPlayer().dropMessage(1, "Your inventory is full (Please rellog)");
                        break;
                    }
                    ...
                    //c.getPlayer().fakeRelog();
                    c.removeClickedNPC();
                    c.getPlayer().dropMessage(1, "items has been returned , please clear your inventory before taking back (Please rellog)");
                }
