        ...
        try {
            ...
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    ...
                    if (iv == null) {
                    	cm.sendOk("对不起，我并没有被管理员设置可使用，如果你觉得我应该工作的，那就请你汇报给管理员.rn" +
                    			"我的ID编号 #r + npc + #k."); 
                        dispose(c);
                        return;
                    }
            ...
            System.err.println("NPC 脚本执行出错, NPC ID : " + npc + "." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "NPC 脚本执行出错, NPC ID : " + npc + "." + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
    ...
    public final void action(final MapleClient c, final byte mode, final byte type, final int selection) {
        if (mode != -1) {
		    ...
		    c.setClickedNPC();
                ...
                System.err.println("NPC 脚本执行出错. NPC ID : " + cm.getNpc() + ":" + e);
                dispose(c);
                FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "NPC 脚本执行出错, NPC ID : " + cm.getNpc() + "." + e);
            } finally {
                lock.unlock();
            }
        }
